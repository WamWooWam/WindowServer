 ...
 */
export async function FillRgn(hDC: HDC, hrgn: HRGN): Promise<boolean> {
    return (await Gdi32.SendMessage({
        nType: GDI32.FillRgn,
        data: { hDC, hrgn }
    })).data as boolean;
...
export async function GetDeviceCaps(hDC: HANDLE, nIndex: number): Promise<number> {
    return (await Gdi32.SendMessage({
...
export async function CreateFontIndirect(lf: LOGFONT): Promise<HANDLE> {
    return (await Gdi32.SendMessage({
