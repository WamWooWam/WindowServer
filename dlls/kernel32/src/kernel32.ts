...
import { IDX_LAST_ERROR, IDX_PID, STD_ERROR_HANDLE, STD_INPUT_HANDLE, } from "./types/kernel32.types.js";
import KERNEL32, {
    CLOSE_HANDLE,
    CREATE_DIRECTORY,
...
let Kernel32: Subsystem;
let K32Memory: Uint32Array;
...
export function MulDiv(nNumber: number, nNumerator: number, nDenominator: number): number {
    return Math.floor(nNumber * nNumerator / nDenominator);
}

export async function Kernel32Initialize(): Promise<void> {
    Kernel32 = await NtRegisterSubsystem(SUBSYS_KERNEL32, Kernel32_HandleMessage, 4096);
