...
export default class FsFile extends NtFile {
    ...
    constructor(
        ...
        this.path = NtFileNameToUnix(lpFileName, hOwner);
    }
    ...
    async open() {
        ...
        this.dwLength = await new Promise((resolve, reject) => {
    ...
    async read(buffer: Uint8Array, offset: number, length: number) {
        const { fs, Buffer } = NtGetFileSystemGlobal();
    ...
    async seek(position: number, origin: 'begin' | 'current' | 'end') {
        switch (origin) {
            ...
            case 'current':
                ...
                break;
            case 'end':
                this.position = this.dwLength + position;
                break;
        }
    }
